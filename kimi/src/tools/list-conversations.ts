...
import { z } from 'zod';
import { listConversations as fetchConversations } from '../kimi-api.js';
...
export const listConversations = defineTool({
  ...
  description:
    "List the account's Kimi chat conversations, newest first — the same list the Kimi sidebar shows. Returns conversation IDs, titles and URLs. Use a returned id with get_conversation or send_message.",
  summary: 'List recent Kimi conversations',
  icon: 'list',
  group: 'Conversations',
  input: z.object({
    limit: z
