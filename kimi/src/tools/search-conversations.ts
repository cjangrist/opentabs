...
export const searchConversations = defineTool({
  ...
  description:
    ...
    'It matches both titles and message bodies, wraps hits in <em> markup (stripped here), and pages with the same real pageToken cursor as list_conversations, ' +
    'so results ARE walkable across a page boundary. ' +
    'NOTE: FeedService/ListFeeds also accepts a `query` and silently ignores it — it re-ranks but still returns every chat — so search is deliberately routed through ListChats instead. ' +
