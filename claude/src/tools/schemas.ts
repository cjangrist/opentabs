...
// claude.ai returns each message as an ordered list of content blocks
// (text / thinking / tool_use / tool_result / token_budget). Taking only the
// first text block silently truncates every assistant turn that used a tool to
// its opening sentence, so concatenate every text block in order instead.
const joinTextBlocks = (content: RawMessage['content']): string =>
  (content ?? [])
...
export const mapMessage = (m: RawMessage) => ({
  uuid: m.uuid ?? '',
  text: m.text || joinTextBlocks(m.content) || '',
  sender: m.sender ?? '',
  index: m.index ?? 0,
